...
import {
  ...
  MoreHorizontal, Mail, ExternalLink,
} from 'lucide-react'
...
const schema = z.object({
  ...
  oportunidadId: z.string().optional(),   // ← fix: optional en el schema
...
function ClienteModal({
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormValues>({
    ...
    defaultValues: cliente
      ? {
          ...
          oportunidadId: cliente.oportunidadId ?? '',
  ...
  async function onSubmit(values: FormValues) {
    // Construir payload limpiando undefined
    const payload: Omit<Cliente, 'id' | 'creadoEn' | 'actualizadoEn'> = {
      nombre:        values.nombre,
      empresa:       values.empresa,
      email:         values.email,
      sector:        values.sector ?? '',
      estado:        values.estado,
      oportunidadId: values.oportunidadId ?? '',   // ← siempre string
      contactos,
    }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-5 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className={cn('flux-input', errors.empresa && 'border-flux-danger')}
            ...
            <div>
              ...
              <input type="email" className={cn('flux-input', errors.email && 'border-flux-danger')}
          ...
          <div>
                ...
                {contactos.map((c, i) => (
                  <div key={i} className="flex items-center gap-3 px-3 py-2 bg-flux-surface border border-flux-border rounded-lg">
                    ...
                    <button type="button" onClick={() => removeContacto(i)}
                      className="text-flux-text3 hover:text-flux-danger transition-colors">
                      <X size={12} />
                    </button>
                  </div>
                ))}
            ...
            <div className="bg-flux-surface border border-flux-border rounded-xl p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input className="flux-input text-xs" placeholder="Nombre *"
                  value={nuevoContacto.nombre}
                  onChange={e => setNuevoContacto(p => ({ ...p, nombre: e.target.value }))} />
                <input className="flux-input text-xs" placeholder="Email *"
                  value={nuevoContacto.email}
                  onChange={e => setNuevoContacto(p => ({ ...p, email: e.target.value }))} />
                <input className="flux-input text-xs" placeholder="Cargo"
                  value={nuevoContacto.cargo}
                  onChange={e => setNuevoContacto(p => ({ ...p, cargo: e.target.value }))} />
                <input className="flux-input text-xs" placeholder="Teléfono"
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={isSubmitting} className="btn-primary flex items-center gap-2">
              {isSubmitting && (
                <div className="w-3.5 h-3.5 border-2 border-flux-bg border-t-transparent rounded-full animate-spin" />
              )}
                    ...
                    <div className="relative" onClick={e => e.stopPropagation()}>
                      ...
                      {menuId === cliente.id && (
                        <div className="absolute top-full right-0 mt-1 z-20 bg-flux-card border border-flux-border rounded-xl shadow-card-hover py-1 min-w-[130px]">
                          <button
                            onClick={() => { setModal(cliente); setMenuId(null) }}
                  ...
                  <Link href={`/clientes/${cliente.id}`} className="block">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="min-w-0">
                        <p className="font-medium text-flux-text1 group-hover:text-flux-white transition-colors truncate">
                          {cliente.empresa}
                        </p>
