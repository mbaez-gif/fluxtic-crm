...
import {
  Plus, Upload, Trash2, Search, X, TrendingDown,
} from 'lucide-react'
...
export default function GastosPage() {
  ...
  // Fix: Array.from instead of spread operator on Set (TS target compatibility)
  const periodos = Array.from(new Set(gastos.map(g => g.periodo))).sort().reverse()
    ...
    <div className="animate-fade-in">
      ...
      <div className="px-4 md:px-8 pb-10 space-y-4 pt-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select className="flux-input w-auto text-sm" value={periodo}
                  ...
                  {filtered.map(g => (
                    <tr key={g.id} className="border-b border-flux-border/50 hover:bg-flux-muted/20 transition-colors">
                      <td className="px-4 py-3">
                        <p className="font-medium text-flux-text1">{g.descripcion}</p>
                      ...
                      <td className="px-4 py-3 font-medium text-flux-white whitespace-nowrap">
                        {g.moneda && g.moneda !== 'ARS' ? `${g.moneda} ` : '$'}
